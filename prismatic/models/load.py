...
from prismatic.models.vlas import OpenVLA, OpenVLA_flip_dino
...
def load_vla_custom(
    ...
    # Load VLM using `from_pretrained` (clobbers HF syntax... eventually should reconcile)
    overwatch.info(f"Loading VLA [bold blue]{model_cfg.model_id}[/] from Checkpoint")
    vla = OpenVLA_flip_dino.from_pretrained(
        checkpoint_pt,
        model_cfg.model_id,
        vision_backbone,
        llm_backbone,
    ...
    # result = vla.vision_backbone.dino_featurizer.load_state_dict(depth_anything.pretrained.state_dict()) 
    # total_keys = len(vla.vision_backbone.dino_featurizer.state_dict().keys())
    # missing_keys = len(result.missing_keys)
    # print(f"Loaded {total_keys - missing_keys} keys from DepthAnything")
    # del depth_anything

    return vla
def load_vla_custom_both(
    model_id_or_path: Union[str, Path],
    hf_token: Optional[str] = None,
    cache_dir: Optional[Union[str, Path]] = None,
    load_for_training: bool = False,
    ...
    model = timm.create_model("vit_large_patch14_reg4_dinov2.lvd142m", pretrained=True, num_classes=0, img_size=224)
    vla.vision_backbone.dino_featurizer.load_state_dict(model.state_dict())
    del model 
    model = timm.create_model("vit_so400m_patch14_siglip_224", pretrained=True, num_classes=0, img_size=224)
    vla.vision_backbone.siglip_featurizer.load_state_dict(model.state_dict())
    del model 
    return vla

def load_vla_custom_alpha(
    model_id_or_path: Union[str, Path],
    hf_token: Optional[str] = None,
    cache_dir: Optional[Union[str, Path]] = None,
    load_for_training: bool = False,
    ...
    """Loads a pretrained OpenVLA from either local disk or the HuggingFace Hub."""
    overwatch.info(f"Loading VLA with model merging alpha{alpha}")
    print(f"Loading VLA with model merging alpha{alpha}")
    ...
    model = timm.create_model("vit_large_patch14_reg4_dinov2.lvd142m", pretrained=True, num_classes=0, img_size=224)
    new_state_dict = vla.vision_backbone.dino_featurizer.state_dict()
    for key in new_state_dict.keys():
        new_state_dict[key] = (1-alpha) * new_state_dict[key] + (alpha) * model.state_dict()[key]
    vla.vision_backbone.dino_featurizer.load_state_dict(new_state_dict)
    del model, new_state_dict
    model = timm.create_model("vit_so400m_patch14_siglip_224", pretrained=True, num_classes=0, img_size=224)
    new_state_dict = vla.vision_backbone.siglip_featurizer.state_dict()
    for key in new_state_dict.keys():
        new_state_dict[key] = (1-alpha) * new_state_dict[key] + (alpha) * model.state_dict()[key]
    vla.vision_backbone.siglip_featurizer.load_state_dict(model.state_dict())
    del model, new_state_dict
